...
# Default to library-based ptpd-app binary
BINARY_NAME="ptpd-app"
BINARY="$PROJECT_DIR/build-cmake/macos-ptpd-app-debug/src-app/$BINARY_NAME"
...
stop_ptpd_processes() {
    ...
    local PTP_PAT='(^|/)(ptpd2?|ptpd[-_][Aa]pp)([[:space:]]|$)'
    ...
    sudo pkill -f '(^|/)(ptpd2?|ptpd[-_][Aa]pp)([[:space:]]|$)' 2>/dev/null || true

    # 5) Port-based cleanup: forcibly kill anything still holding PTP ports 319/320
    local port_pids
    port_pids="$(sudo lsof -ti :319 -ti :320 2>/dev/null || true)"
    if [ -n "$port_pids" ]; then
        print_status "🔌 Force-killing processes still holding PTP ports 319/320: $port_pids"
        echo "$port_pids" | xargs sudo kill -9 2>/dev/null || true
        sleep 0.5
    fi

    # 6) Sanity check: show anything still around
