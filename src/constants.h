...
/* FIXME: make these parameterized, either through command-line options or make
 * variables */
/* wowczarek@25oct15: fixed: product description suffix is
 * variables:product_description */
/* user description is ptpengine:port_description */
#define MANUFACTURER_ID_OUI0 0xFF
#define MANUFACTURER_ID_OUI1 0xFF
#define MANUFACTURER_ID_OUI2 0xFF
#define PROTOCOL "IEEE 802.3"
#define PRODUCT_DESCRIPTION "ptpd;" PACKAGE_VERSION ";%s"
#ifndef PTPD_SLAVE_ONLY

#define USER_VERSION PACKAGE_VERSION

#else

#define USER_VERSION PACKAGE_VERSION "-slaveonly"

#endif /* PTPD_SLAVE_ONLY */

#define REVISION ";;" USER_VERSION ""
...
#define DEFAULT_DELAYREQ_INTERVAL                                              \
  0 /* new value from page 237 of the standard */

#define DEFAULT_SYNC_INTERVAL                                                  \
  0 /* -7 in 802.1AS */ /* from page 237 of the standard */
/* number of announces we need to lose until a time out occurs. Thus it is 12
 * seconds */
#define DEFAULT_ANNOUNCE_RECEIPT_TIMEOUT 6 /* 3 by default */

#define DEFAULT_FAILURE_WAITTIME                                               \
  10 /* sleep for 10 seconds on failure once operational */
...
248     Default. This clockClass shall be used if none of the other clockClass
definitions apply. 13      Shall designate a clock that is synchronized to an
application-specific source of time. The timescale distributed shall be ARB. A
clockClass 13 clock shall not be a slave to another clock in the domain.
*/
...
#define DEFAULT_PRIORITY2                                                      \
  128 /* page 238, default priority is the midpoint, to allow easy control of  \
         the BMC algorithm */

/* page 238:  τ, see 7.6.3.2: The default initialization value shall be 1.0 s.
 */
// #define DEFAULT_CLOCK_VARIANCE 	        28768 /* To be determined in
// 802.1AS. */
...
enum { UDP_IPV4 = 1, UDP_IPV6, IEEE_802_3, DeviceNet, ControlNet, PROFINET };
 ...
 */
enum { GET = 0, SET, RESPONSE, COMMAND, ACKNOWLEDGE };
...
enum { TSP_DEFAULT = 0x00, TSP_ETHERNET_AVB = 0x01 };
...
#define MISSED_MESSAGES_MAX                                                    \
  20 /* how long we wait to trigger a [sync/delay] receipt alarm */
