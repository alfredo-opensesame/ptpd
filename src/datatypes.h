...
typedef struct {
  ...
  uint32_t
      foreignOverflows; // implement me! /* how many times the FMR was full */
  ...
  uint32_t discardedMessages; /* only messages we shouldn't be receiving -
                                 ignored from self don't count */
  uint32_t unknownMessages;   /* unknown type - also increments discarded */
  uint32_t ignoredAnnounce;   /* ignored Announce messages: acl / security /
                                 preference */
  uint32_t aclTimingMessagesDiscarded; /* Timing messages discarded by access
                                          lists */
  uint32_t aclManagementMessagesDiscarded; /* Timing messages discarded by
                                              access lists */
  ...
  uint32_t versionMismatchErrors;  /* V1 received, V2 expected - also increments
                                      discarded */
  uint32_t domainMismatchErrors;   /* different domain than configured - also
                                      increments discarded */
  uint32_t sequenceMismatchErrors; /* mismatched sequence IDs - also increments
                                      discarded */
  uint32_t delayMechanismMismatchErrors; /* P2P received, E2E expected or vice
                                            versa - incremets discarded */
  uint32_t consecutiveSequenceErrors; /* number of consecutive sequence mismatch
                                         errors */

  /* unicast sgnaling counters */
  uint32_t unicastGrantsRequested; /* slave: how many we requested, master: how
                                      many requests we received */
  uint32_t unicastGrantsGranted; /* slave: how many we got granted, master: how
                                    many we granted */
  uint32_t unicastGrantsDenied;  /* slave: how many we got denied, master: how
                                    many we denied */
...
#ifdef PTPD_STATISTICS
  uint32_t
      delayMSOutliersFound; /* Number of outliers found by the delayMS filter */
  uint32_t
      delaySMOutliersFound; /* Number of outliers found by the delaySM filter */
#endif                      /* PTPD_STATISTICS */
  uint32_t
      maxDelayDrops; /* number of samples dropped due to maxDelay threshold */
...
typedef struct {
  Boolean activity; /* periodic check, updateClock sets this to let the watchdog
                       know we're holding clock control */
  Boolean available;    /* flags that we can control the clock */
  Boolean granted;      /* upstream watchdog will grant this when we're the best
                           provider */
...
typedef struct {
  ...
  Boolean cancelCount;        /* how many times we sent the cancel message while
                                 waiting for ack */
  ...
  UInteger32 intervalCounter; /* used as a modulo counter to allow different
                                 message rates for different slaves */
  Boolean expired;            /* TRUE -> grant has expired */
  Boolean granted; /* master: we have granted this, slave: we have been granted
                      this */
  UInteger32 timeLeft;    /* countdown timer for aging out grants */
  UInteger16 messageType; /* message type this grant is for */
  UnicastGrantTable
      *parent; /* parent entry (that has transportAddress and portIdentity */
  Boolean receiving; /* keepalive: used to detect if message of this type is
                        being received */
} UnicastGrantData;
...
struct UnicastGrantTable {
  ...
  UInteger8
      domainNumber; /* domain of the master - as used by Telecom Profile */
  UInteger8 localPreference; /* local preference - as used by Telecom profile */
  PortIdentity
      portIdentity; /* master: port ID of grantee, slave: portID of grantor */
  UnicastGrantData
      grantData[PTP_MAX_MESSAGE_INDEXED]; /* master: grantee's grants, slave:
                                             grantor's grant status */
  UInteger32 timeLeft; /* time until expiry of last grant (max[grants.timeLeft].
                          when runs out and no renewal, entry can be re-used */
...
/* Unicast destination configuration: Address, domain, preference, last Sync
 * timestamp sent */
typedef struct {
  Integer32 transportAddress; /* destination address */
  UInteger8 domainNumber; /* domain number - for slaves with masters in multiple
                             domains */
...
typedef struct {
  ...
  /* optional BMC extension: accept any domain, prefer configured domain, prefer
   * lower domain */
  Boolean anyDomain;
  ...
  Boolean
      snmpEnabled; /* SNMP subsystem enabled / disabled even if compiled in */
  Boolean snmpTrapsEnabled; /* enable sending of SNMP traps (requires alarms
                               enabled) */
  Boolean alarmsEnabled;    /* enable support for alarms */
  int alarmMinAge;          /* minimal alarm age in seconds (from set to clear
                               notification) */
  int alarmInitialDelay;    /* initial delay before we start processing alarms;
                               example:  */
  ...
  Boolean
      disableUdpChecksums; /* disable UDP checksum validation where supported */
  ...
  Boolean unicastNegotiationListening; /* Master: Reply to signaling messages
                                          when in LISTENING */
  Boolean disableBMCA;      /* used to achieve master-only for unicast */
  Boolean unicastAcceptAny; /* Slave: accept messages from all GMs, regardless
                               of grants */
  /*
   ...
   */
  UInteger16 unicastPortMask; /* port mask to apply to portNumber when using
                                 negotiation */
...
typedef struct {
  ...
  Boolean record_update; /* should we run bmc() after receiving an announce
                            message? */

  Boolean disabled; /* port is permanently disabled */

  /* unicast grant table - our own grants or our slaves' grants or grants to
   * peers */
  ...
  /* another index to match unicast Sync with FollowUp when we can't capture the
   * destination address of Sync */
  ...
  /*
          20110630: These variables were deprecated in favor of the ones that
     appear in the stats log (delayMS and delaySM)
  ...
  Integer32 stabilisation_time; /* How long (seconds) it took to stabilise the
                                   clock */
  ...
  Integer32 lastSyncDst; /* destination address for last sync, so we know where
                            to send the followUp - last resort: we should
                            capture the dst address ourselves */
  Integer32
      lastPdelayRespDst; /* captures the destination address of last pdelayResp
                            so we know where to send the pdelayRespfollowUp */
  ...
  /* "panic mode" support */
  Boolean
      panicMode; /* in panic mode - do not update clock or calculate offsets */
