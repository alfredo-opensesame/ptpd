...
enum { RANGECHECK_NONE, RANGECHECK_RANGE, RANGECHECK_MIN, RANGECHECK_MAX };
...
#define CFGOP_PARSE                                                            \
  1 << 0 /* parse all config options, return success/failure */
...
#define CFGOP_RESTART_FLAGS                                                    \
  1 << 5 /* return subsystems affected by config changes */

Boolean loadConfigFile(dictionary **, RunTimeOpts *);
void loadCommandLineKeys(dictionary *, int, char **);
Boolean loadCommandLineOptions(RunTimeOpts *, dictionary *, int, char **,
                               Integer16 *);
