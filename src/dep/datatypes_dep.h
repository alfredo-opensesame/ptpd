 ...
 *
 * The FIR filtering of the offset from master input is a simple, two-sample
 * average
 */
typedef struct {
 ...
 *
 * It is a variable cutoff/delay low-pass, infinite impulse response (IIR)
 * filter.
 *
 *  The one-way delay filter has the difference equation: s*y[n] - (s-1)*y[n-1]
 * = x[n]/2 + x[n-1]/2, where increasing the stiffness (s) lowers the cutoff and
 * increases the delay.
 */
typedef struct {
  Integer32 nsec_prev, y;
...
#endif
  /* source address of last received packet - used for unicast replies to Delay
   * Requests */
  Integer32 lastSourceAddr;
  /* destination address of last received packet - used for unicast FollowUp for
   * multiple slaves*/
