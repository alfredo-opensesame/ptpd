...
// Syslog ordering. We define extra debug levels above LOG_DEBUG for internal
// use - but message() doesn't pass these to SysLog
...
#define PERROR(x, ...)                                                         \
  logMessage(LOG_ERR, x "      (strerror: %m)\n", ##__VA_ARGS__)
...
#define EMERGENCY_LOCAL_ID(o, x, ...)                                          \
  EMERGENCY(LOCAL_PREFIX ".%s: " x, o->id, ##__VA_ARGS__)
#define ALERT_LOCAL_ID(o, x, ...)                                              \
  ALERT(LOCAL_PREFIX ".%s: " x, o->id, ##__VA_ARGS__)
#define CRITICAL_LOCAL_ID(o, x, ...)                                           \
  CRITICAL(LOCAL_PREFIX ".%s: " x, o->id, ##__VA_ARGS__)
#define ERROR_LOCAL_ID(o, x, ...)                                              \
  ERROR(LOCAL_PREFIX ".%s: " x, o->id, ##__VA_ARGS__)
#define PERROR_LOCAL_ID(o, x, ...)                                             \
  PERROR(LOCAL_PREFIX ".%s: " x, o->id, ##__VA_ARGS__)
#define WARNING_LOCAL_ID(o, x, ...)                                            \
  WARNING(LOCAL_PREFIX ".%s: " x, o->id, ##__VA_ARGS__)
#define NOTIFY_LOCAL_ID(o, x, ...)                                             \
  NOTIFY(LOCAL_PREFIX ".%s: " x, o->id, ##__VA_ARGS__)
#define NOTICE_LOCAL_ID(o, x, ...)                                             \
  NOTICE(LOCAL_PREFIX ".%s: " x, o->id, ##__VA_ARGS__)
#define INFO_LOCAL_ID(o, x, ...)                                               \
  INFO(LOCAL_PREFIX ".%s: " x, o->id, ##__VA_ARGS__)
...
#define DBGV_LOCAL_ID(o, x, ...)                                               \
  DBGV(LOCAL_PREFIX ".%s:" x, o->id, ##__VA_ARGS__)
#else
#define DBGV(x, ...)
#define DBGV_LOCAL(x, ...)
...
#define DBG2_LOCAL_ID(o, x, ...)                                               \
  DBG2(LOCAL_PREFIX ".%s:" x, o->id, ##__VA_ARGS__)
...
#define getFlag(x, y)                                                          \
  !!(*(UInteger8 *)((x) + ((y) < 8 ? 1 : 0)) & (1 << ((y) < 8 ? (y) : (y) - 8)))
#define setFlag(x, y)                                                          \
  (*(UInteger8 *)((x) + ((y) < 8 ? 1 : 0)) |= 1 << ((y) < 8 ? (y) : (y) - 8))
#define clearFlag(x, y)                                                        \
  (*(UInteger8 *)((x) + ((y) < 8 ? 1 : 0)) &= ~(1 << ((y) < 8 ? (y) : (y) - 8)))
/** \}*/
...
Boolean msgUnpackManagement(Octet *buf, MsgManagement *, MsgHeader *,
                            PtpClock *ptpClock, const int tlvOffset);
Boolean msgUnpackSignaling(Octet *buf, MsgSignaling *, MsgHeader *,
                           PtpClock *ptpClock, const int tlvOffset);
...
int unpackMMAnnounceReceiptTimeout(Octet *buf, int, MsgManagement *,
                                   PtpClock *);
...
int unpackMMTraceabilityProperties(Octet *buf, int, MsgManagement *,
                                   PtpClock *);
...
int unpackMMUnicastNegotiationEnable(Octet *buf, int, MsgManagement *,
                                     PtpClock *);
...
int unpackMMLogMinPdelayReqInterval(Octet *buf, int, MsgManagement *,
                                    PtpClock *);
...
void unpackSMAcknowledgeCancelUnicastTransmission(Octet *buf, MsgSignaling *,
                                                  PtpClock *);
...
UInteger16 msgPackManagementResponse(Octet *buf, MsgHeader *, MsgManagement *,
                                     PtpClock *);
/** \}*/
...
ssize_t netSendEvent(Octet *, UInteger16, NetPath *, const RunTimeOpts *,
                     Integer32, TimeInternal *);
ssize_t netSendGeneral(Octet *, UInteger16, NetPath *, const RunTimeOpts *,
                       Integer32);
ssize_t netSendPeerGeneral(Octet *, UInteger16, NetPath *, const RunTimeOpts *,
                           Integer32);
ssize_t netSendPeerEvent(Octet *, UInteger16, NetPath *, const RunTimeOpts *,
                         Integer32, TimeInternal *);
...
void updatePeerDelay(one_way_delay_filter *, const RunTimeOpts *, PtpClock *,
                     TimeInternal *, Boolean);
void updateDelay(one_way_delay_filter *, const RunTimeOpts *, PtpClock *,
                 TimeInternal *);
void updateOffset(TimeInternal *, TimeInternal *, offset_from_master_filter *,
                  const RunTimeOpts *, PtpClock *, TimeInternal *);
