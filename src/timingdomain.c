...
static int cmpTimingService(TimingService *a, TimingService *b,
                            Boolean useableOnly);
...
static int ptpServiceUpdate(TimingService *service) {
  ...
  ptpClock->counters.messageSendRate =
      ptpClock->netPath.sentPackets / service->updateInterval;
  ptpClock->counters.messageReceiveRate =
      ptpClock->netPath.receivedPackets / service->updateInterval;
  ...
  if (rtOpts->leapSecondHandling == LEAP_ACCEPT) {
    ...
    if (!ptpClock->leapSecondInProgress) {
      if (leapInsert && !clockStatus->leapInsert) {
        DBG_LOCAL_ID(service,
                     "STA_INS in kernel but not in PTP: withdrawing\n");
        unsetTimexFlags(STA_INS, TRUE);
      }

      if (leapDelete && !clockStatus->leapDelete) {
        DBG_LOCAL_ID(service,
                     "STA_DEL in kernel but not in PTP: withdrawing\n");
        unsetTimexFlags(STA_DEL, TRUE);
      }
    }

    if (!leapInsert && clockStatus->leapInsert) {
      WARNING("Leap second pending! Setting clock to insert one second at "
              "midnight\n");
      setTimexFlags(STA_INS, FALSE);
    }

    if (!leapDelete && clockStatus->leapDelete) {
      WARNING("Leap second pending! Setting clock to delete one second at "
              "midnight\n");
      setTimexFlags(STA_DEL, FALSE);
    }
  } else {
    if (leapInsert || leapDelete) {
      DBG("leap second handling is not set to accept - withdrawing kernel leap "
          "flags!\n");
      unsetTimexFlags(STA_INS, TRUE);
      unsetTimexFlags(STA_DEL, TRUE);
    }
...
static int ntpServiceInit(TimingService *service) {
  ...
  if (ntpInit(config, controller)) {
    ...
    FLAGS_UNSET(service->flags, TIMINGSERVICE_OPERATIONAL);
    FLAGS_UNSET(service->flags, TIMINGSERVICE_AVAILABLE);
    INFO_LOCAL_ID(service,
                  "Could not start NTP service. Will keep retrying.\n");
    return 0;
  }
}
...
static int ntpServiceAcquire(TimingService *service) {
  ...
  if (!config->enableControl) {
    if (!controller->requestFailed) {
      WARNING_LOCAL_ID(service,
                       "control disabled, cannot acquire clock control\n");
    }
  ...
  default:
    if (!controller->requestFailed) {
      WARNING_LOCAL_ID(service,
                       "failed to acquire clock control - clock may drift!\n");
    }
    controller->requestFailed = TRUE;
    return 0;
  }
...
static int ntpServiceRelease(TimingService *service, int reason) {
  ...
  if (!config->enableControl) {
    if (!controller->requestFailed) {
      WARNING_LOCAL_ID(service,
                       "control disabled, cannot release clock control\n");
    }
    controller->requestFailed = TRUE;
    return 1;
  }
  ...
  default:
    if (!controller->requestFailed) {
      WARNING_LOCAL_ID(service,
                       "failed to release clock control, reason: %s - clock "
                       "may be unstable!\n",
                       reasonToString(reason));
    }
    controller->requestFailed = TRUE;
...
static int ntpServiceUpdate(TimingService *service) {
  ...
  if (res != INFO_YES && res != INFO_NO) {
    if (!controller->checkFailed) {
      WARNING_LOCAL_ID(service,
                       "Could not verify NTP status  - will keep checking\n");
    }
...
static int ntpServiceClockUpdate(TimingService *service) { return 1; }
...
static int timingDomainUpdate(TimingDomain *domain) {
  ...
  for (i = 0; i < domain->serviceCount; i++) {
    ...
    /* each TimingService can have a different update interval: skip when not
     * due */
    ...
    if (service->flags & TIMINGSERVICE_OPERATIONAL) {
      ...
      if ((service->flags & TIMINGSERVICE_AVAILABLE) &&
        ...
        if ((service == domain->current) && (service->holdTimeLeft <= 0)) {
  ...
  if ((best != domain->current) && (domain->electionLeft == 0)) {
    ...
    if (FLAGS_ARESET(best->flags,
                     TIMINGSERVICE_OPERATIONAL | TIMINGSERVICE_AVAILABLE)) {
